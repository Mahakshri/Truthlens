...
function getLocalFallbackAnalysis(title: string, content: string, category: string): any {
  ...
  // Require a minimum title length before allowing substring matches, otherwise a short/generic
  // rule title (e.g. "election") would silently override the verdict for every unrelated article
  ...
  const matchingRule = customRules.find(rule => {
    const rTitle = (rule.title || '').toLowerCase().trim();
    if (rTitle.length < MIN_MATCH_LENGTH) return false;
...
You must return a raw JSON object strictly adhering to the specified schema, with no additional formatting or wrapper blocks.${dynamicCalibration}`;
    ...
    // Sanity-check the score against the risk level WITHOUT discarding the model's real output.
    // We only clamp the score into the valid range for its bucket if it's missing, non-numeric,
    // or on the wrong side of the boundary — and we clamp to the nearest valid edge, never
    // to a random number. This preserves whatever nuance the model actually returned.
    const risk = (analysis.risk_level || 'Low').toLowerCase();
    const hasValidScore = typeof analysis.authenticity_score === 'number' && !Number.isNaN(analysis.authenticity_score);

    const RANGES: Record<string, [number, number]> = {
      high: [0, 30],    // Fake/rumor: authenticity must stay low
      medium: [25, 70],  // Misleading/exaggerated: mid-range
      low: [75, 100],    // Verified genuine: authenticity must stay high
    };
    const [min, max] = RANGES[risk] || RANGES.low;

    if (!hasValidScore) {
      // Model gave us nothing usable — use the midpoint of the expected range, not a random pick.
      analysis.authenticity_score = Math.round((min + max) / 2);
    } else if (analysis.authenticity_score < min) {
      analysis.authenticity_score = min;
    } else if (analysis.authenticity_score > max) {
      analysis.authenticity_score = max;
    }
    // else: the model's real score is within the valid range for its risk level — leave it untouched.

    analysis.authenticity_score = Math.max(0, Math.min(100, Math.round(analysis.authenticity_score)));
    analysis.fabricated_percentage = 100 - analysis.authenticity_score;

    // Clamp the remaining 0-100 fields in case the model returns an out-of-range or non-numeric value.
    const clampPercent = (val: any, fallback: number) =>
      typeof val === 'number' && !Number.isNaN(val) ? Math.max(0, Math.min(100, Math.round(val))) : fallback;
    analysis.clickbait_index = clampPercent(analysis.clickbait_index, 20);
    analysis.ai_generated_probability = clampPercent(analysis.ai_generated_probability, 20);
