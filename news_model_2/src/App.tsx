...
export default function App() {
  ...
  // Model Retraining and Calibration State
  ...
  const [retrainingStep, setRetrainingStep] = useState<string>('');
  const [showCalibrationForm, setShowCalibrationForm] = useState<boolean>(false);

  // Calibration Form Inputs
  const [corrTitle, setCorrTitle] = useState<string>('');
  const [corrVerdict, setCorrVerdict] = useState<string>('Fabricated / Rumor');
  const [corrExplanation, setCorrExplanation] = useState<string>('');
  const [corrFacts, setCorrFacts] = useState<string>('');
  const [corrSuccess, setCorrSuccess] = useState<string>('');
  ...
  // Load verified news and training status from backend on mount
  useEffect(() => {
    fetchNews();
    fetchTrainingStatus();
  }, []);
  ...
  const handleRetrainModel = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!corrTitle || !corrExplanation || !corrFacts) {
      setCorrError('Please fill out all fields for model recalibration.');
      return;
    }
    ...
    // Phase 1: Compile Custom Rule Dataset
    setRetrainingStep('Phase 1/4: Gathering dataset corrections...');
    await new Promise(r => setTimeout(r, 600));

    // Phase 2: Feature Extraction
    setRetrainingStep('Phase 2/4: Extracting linguistic cues & target weights...');
    await new Promise(r => setTimeout(r, 800));

    // Phase 3: Submit to pipeline
    setRetrainingStep('Phase 3/4: Fine-tuning Gemini system instructions...');
    try {
      ...
      if (res.ok) {
        // Phase 4: Validate
        setRetrainingStep('Phase 4/4: Performing cross-validation on verification test sets...');
        await new Promise(r => setTimeout(r, 700));

        setTrainingStatus(data.stats);
        setCorrSuccess('Model recalibration completed successfully! The model has been fine-tuned and corrected on this topic.');
        setCorrTitle('');
        setCorrExplanation('');
        setCorrFacts('');
        // Refresh feed and stats to show any corrected state
        fetchNews();
        fetchTrainingStatus();
      } else {
        setCorrError(data.error || 'Failed to submit training parameters.');
      }
    } catch (err) {
      setCorrError('Could not connect to model training service.');
    } finally {
  ...
  const handleClearCalibration = async () => {
    if (!window.confirm('Are you sure you want to revert the model to baseline factory defaults? This clears all corrective rules.')) {
      return;
    }
    try {
      const res = await fetch('/api/retrain/clear', { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
        setTrainingStatus(data.stats);
        setCorrSuccess('Model reverted to factory defaults successfully!');
        fetchNews();
        fetchTrainingStatus();
      }
    } catch (e) {
      console.error('Error resetting model:', e);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-5 space-y-6">
          ...
          {/* AI Model Calibration & Retraining Dashboard */}
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] bg-slate-100 text-slate-500 font-extrabold uppercase px-2 py-0.5 rounded-md flex items-center gap-1">
                <Sliders size={10} /> Active Pipeline
              </span>
            </div>

            <p className="text-xs text-slate-500 leading-relaxed">
              Correct wrong predictions or refine accuracy directly. By retraining the model on specific topics, your ground-truth corrections calibrate the Gemini system instructions for all future evaluations.
            </p>

            {/* Model stats board */}
            <div className="grid grid-cols-2 gap-2 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/60">
              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Calibrated Accuracy</span>
                <strong className="text-sm font-black text-slate-700">{trainingStatus?.accuracy || 92.4}%</strong>
              </div>
              <div className="space-y-0.5">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Trained Samples</span>
                <strong className="text-sm font-black text-slate-700">{(trainingStatus?.total_training_samples || 150)}</strong>
              </div>
              <div className="space-y-0.5 pt-1.5 border-t border-slate-100">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Custom Rules</span>
                <strong className="text-sm font-black text-emerald-600">+{trainingStatus?.custom_rules_count || 0} active</strong>
              </div>
              <div className="space-y-0.5 pt-1.5 border-t border-slate-100">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Last Retrained</span>
                <strong className="text-[11px] font-extrabold text-slate-500 truncate block">{trainingStatus?.last_trained || 'Baseline'}</strong>
              </div>
            </div>

            {/* Display Active Rules if any exist */}
            {trainingStatus?.active_rules && trainingStatus.active_rules.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                    Active Tuning Overrides ({trainingStatus.active_rules.length})
                  </span>
                  <button
                    onClick={handleClearCalibration}
                    className="text-[10px] font-bold text-rose-600 hover:text-rose-700 hover:underline bg-transparent border-0 cursor-pointer"
                  >
                    Reset defaults
                  </button>
                </div>
                <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 border border-slate-100 p-2 rounded-xl bg-slate-50/50">
                  {trainingStatus.active_rules.map((rule: any, idx: number) => (
                    <div key={idx} className="bg-white border border-slate-100 p-2 rounded-lg text-[11px] font-medium text-slate-700 flex items-start justify-between gap-2 shadow-2xs">
                      <div className="space-y-0.5">
                        <strong className="text-slate-800 font-extrabold block truncate max-w-[200px]">{rule.title}</strong>
                        <span className="text-[9px] font-semibold text-rose-500 bg-rose-50 border border-rose-100 rounded-sm px-1.5 py-0.2 select-none uppercase">
                          {rule.verdict}
                        </span>
                      </div>
                      <span className="text-[8px] text-slate-400 font-normal shrink-0 mt-1">Trained</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="border-t border-slate-100 pt-3">
              {!showCalibrationForm ? (
                <button
                  ...
                  className="w-full bg-slate-50 border border-slate-200 hover:bg-slate-100/80 text-slate-700 font-extrabold text-xs py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <Wrench size={13} />
                  Tune Model / Add Correction Rule
                </button>
              ) : (
                <form onSubmit={handleRetrainModel} className="space-y-3.5">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-bold text-slate-700 flex items-center gap-1">
                      <Wrench size={12} className="text-emerald-600" />
                      Add Retraining Instruction
                    </h3>
                    <button
                      type="button"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., Drinking warm lemon water completely cures diabetes"
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-lg px-3 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none transition-all"
                      required
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    ...
                    <select
                      value={corrVerdict}
                      onChange={(e) => setCorrVerdict(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-lg px-2.5 py-1.5 text-xs text-slate-800 focus:outline-none cursor-pointer"
                    >
                      <option value="Fabricated / Rumor">❌ Fabricated / Rumor</option>
                      <option value="Misleading / Exaggerated">⚠️ Misleading / Exaggerated</option>
                      <option value="Verified / Genuine">✅ Verified / Genuine</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      placeholder="Explain why the prediction was wrong and what is the real fact..."
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-lg px-3 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none transition-all resize-none"
                      required
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 mb-0.5">
                      Verified Facts & Actionable Advice
                    </label>
                    <textarea
                      value={corrFacts}
                      onChange={(e) => setCorrFacts(e.target.value)}
                      rows={2}
                      placeholder="Provide simple actionable truth instructions..."
                      className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-lg px-3 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none transition-all resize-none"
                      required
                    />
                  </div>
                  ...
                  {corrSuccess && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-800 flex items-start gap-1.5">
                      ...
                      <span>{corrSuccess}</span>
                    </div>
                  )}

                  {corrError && (
                    <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-xs text-rose-800 flex items-start gap-1.5">
                  ...
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-bold text-xs py-2.5 rounded-lg transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                  >
                    {isRetraining ? (
                      <>
            ...
            {/* Retraining interactive progression logs */}
            <AnimatePresence>
              {isRetraining && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="bg-slate-900 text-emerald-400 font-mono text-[10px] p-4 rounded-2xl border border-slate-800 space-y-2 shadow-inner"
                >
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
                    <span className="font-bold uppercase tracking-wider text-slate-300">ML Training Log</span>
                  </div>
                  <div className="space-y-1 text-emerald-400/90 leading-relaxed font-semibold">
                    <p className="animate-pulse">{retrainingStep}</p>
                    <p className="text-slate-500 font-normal select-none">
                      &gt; Updating neural network configuration weights...<br />
                      &gt; Realigning system instructional embeddings...
                    </p>
                  </div>
                </motion.div>
              )}
