...
def extract_video_frames(video_path, target_frames=30, image_size=224):
    ...
    if not OPENCV_AVAILABLE:
        print("[!] OpenCV is not installed ('pip install opencv-python'). Using procedural synthetic frames for pipeline demonstration.")
        # Return synthetic frames matching ImageNet distribution
        synthetic = np.random.randn(target_frames, 3, image_size, image_size).astype(np.float32)
        return torch.tensor(synthetic).unsqueeze(0), None # batch size 1
